...
class CreatingContentTypeEventsTest extends TestCase {
    ...
    public function testCreatingAndCreatedEvent()
    {
        ...
        $eventDispatcherMock->expects($this->exactly(2))
            ->method('fire')
            ->withConsecutive(
                array($this->equalTo('contentType.creating'), $this->isType('array')),
                array($this->equalTo('contentType.created'), $this->isType('array'))
            );
