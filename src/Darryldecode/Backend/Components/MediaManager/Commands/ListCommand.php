...
class ListCommand extends Command  {
    ...
    public function handle(Filesystem $filesystem, Repository $config)
    {
        ...
        $response = [];
        $response['files']          = $this->normalizeFilePathsForDisplay($filesystem->files($path));
        $response['directories']    = $filesystem->directories($path);
        $response['base_path']      = $config->get('filesystems.disks.public.url');
    ...
    /**
     * Just a helper that replaces "public/" in file url path
     *
     * @param $files
     * @return array
     */
    protected function normalizeFilePathsForDisplay($files)
    {
        $newFiles = [];

        foreach($files as $file)
        {
            array_push($newFiles,str_replace('public/','',$file));
        }
